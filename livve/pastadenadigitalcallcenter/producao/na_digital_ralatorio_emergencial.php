...
mysql_select_db($database_provider, $provider);
$query_con_sit = sprintf("SELECT * FROM tb_na WHERE (((tb_na.prioridade)='FIO / RAMAL DE SERVIÇO PARTIDO' Or (tb_na.prioridade)='CHAVE FUSÍVEL' Or (tb_na.prioridade)='FALTA DE ENERGIA' Or (tb_na.prioridade)='POSTE QUEBRADO' Or (tb_na.prioridade)='OBJETO ESTRANHO NA REDE' Or (tb_na.prioridade)='DEFEITO NA MEDIÇÃO' Or (tb_na.prioridade)='ARVORE NA REDE' Or (tb_na.prioridade)='DESLIGAMENTO DE EMERGÊNCIA' Or (tb_na.prioridade)='RELIGAÇÃO DE LUZ PELO COD' Or (tb_na.prioridade)='RECL. POR VARIAÇÃO DE TENSÃO') AND ((tb_na.situacao)='EM ANDAMENTO')) ORDER BY distrito ASC", $colname_con_sit);
...
<table width="720" border="1" align="center" cellpadding="0" cellspacing="0" bordercolor="#CCCCCC" bgcolor="#FFFFFF">
  <tr>
    <td class="style48"><div align="center" class="style57">Relat&oacute;rio de Servi&ccedil;os - Emergencial </div></td>
  </tr>
</table><br />
<table width="720" border="1" align="center" cellpadding="0" cellspacing="0" bordercolor="#CCCCCC" bgcolor="#FFFFFF">
  <tr>
